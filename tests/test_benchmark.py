...
from proccesing import reflect_index, replicate, wrap
...
def pillow_filter_channel(channel, kernel, numpy_pad_mode):
    ...
    filtered = pil_image.filter(
        ImageFilter.Kernel(size=(kw, kh), kernel=kernel.astype(float).flatten().tolist(), scale=1, offset=0)
    )
...
def pillow_filter(image, kernel, edge_mode_name):
    ...
    channels = [pillow_filter_channel(image[:,:,c], kernel, numpy_pad_mode) for c in range(image.shape[2])]
